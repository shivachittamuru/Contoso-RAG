...
export const createClaimsTable = (claims: Claim): ClaimDescription => {
    const claimsObj = {};
    let index = 0;
...
export function compareIssuingPolicy(idTokenClaims: Claim, policyToCompare: string): boolean {
    const tfpMatches = Object.prototype.hasOwnProperty.call(idTokenClaims, 'tfp') && 
                     idTokenClaims['tfp'].toLowerCase() === policyToCompare.toLowerCase();
    const acrMatches = Object.prototype.hasOwnProperty.call(idTokenClaims, 'acr') && 
